...
export default function CamaraScreen() {
  ...
  const uploadImage = async () => {
    ...
    if (image) {
      ...
      alert('Imagen cargada exitosamente!'); // Informative feedback

      // Optionally display the uploaded image URL
      console.log('Download URL:', downloadURL);
    } else {
      alert('Seleccione una imagen para cargar.');
    }
  };

  return (
    <View style={styles.container}>
       <Button title="ABRE LA CAMARA" onPress={pickImage} color={"blue"} />
      <Image source={{ uri: image }} style={styles.image} />
      <Button title={uploading ? 'Subiendo...' : 'Subir Imagen'} disabled={uploading} onPress={uploadImage} color={"black"} />
    </View>
  );
}
